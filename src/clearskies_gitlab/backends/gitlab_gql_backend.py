...
# from clearskies import Model, configs, functional
...
# class GitlabGqlBackend(Backend):
...
#         """Return the GQL client."""
#         if hasattr(self, "_client") and self._client:
#             return self._client
#         transport = AIOHTTPTransport(url=self.base_url, headers=self.authentication.headers())
...
#         singular_name = model.destination_name()
...
#             else " ".join(
#                 [column.name for column in model.get_columns().values() if self._allowed_select_column(column)]
#             )
#         )
